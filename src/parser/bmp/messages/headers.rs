    ...
    match peer_type {
        ...
        BmpPeerType::LocalRib => {
            ...
            if peer_addr_bytes != [0u8; 16] {
                warn!("RFC 9069 violation: Local RIB peer address MUST be zero-filled, but found non-zero bytes (parsing BmpPerPeerHeader)");
            }
            ...
            if peer_bgp_id == Ipv4Addr::from(0) && peer_distinguisher != 0 {
                warn!("RFC 9069: Local RIB peer BGP ID should be set to VRF instance router-id for non-global instances (parsing BmpPerPeerHeader)");
            }
