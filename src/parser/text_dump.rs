...
pub struct TextDumpHeader {
    /// BGP table version, when the preamble carries one (absent in route-views dumps).
    pub table_version: Option<u64>,
    /// Local router ID, used as the `peer_ip` of parsed elements (absent in route-views dumps).
    pub router_id: Option<IpAddr>,
    /// Local AS number, used as the `peer_asn` of parsed elements (absent in route-views dumps).
...
///
/// Reads up to 256 bytes and returns them alongside the detection result so
/// callers can chain the buffered bytes into the actual parser.
...
///
/// Consumes lines up to and including the column header line
/// (`Network / Next Hop / Metric / LocPrf / Weight / Path`); the reader is left
/// positioned at the first route line.
...
/// Parse a complete Cisco `sh ip bgp` text dump into [`BgpElem`]s.
///
...
mod tests {
    ...
    fn test_detect_text_dump_negative() {
        let data = [0x00, 0x00, 0x00, 0x00, 0x00, 0x0d, 0x00, 0x01];
