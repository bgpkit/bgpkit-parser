...
`bgpkit-parser` is bundled with a utility command line tool `bgpkit-parser`.
...
Options:
  ...
  -L, --level <LEVEL>            Output level: elems (per-prefix) or records (MRT records) [default: elems]
      ...
      --hex                      Include each record's raw bytes as hex: a HEX: line in text blocks, a hex field in JSON records
...
- [RFC 10039](https://datatracker.ietf.org/doc/html/rfc10039): BGP Domain Path (D-PATH) Attribute
