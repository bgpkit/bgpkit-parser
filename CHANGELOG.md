...
* **`rislive::messages::Announcement::next_hop` is now the verbatim RIS Live string** (`String`, was `IpAddr`): the field can carry a comma-joined RFC 2545 global + link-local pair (see Fixed below), so like `prefixes` it now keeps the wire value — parse it with `NextHopAddress`'s new `FromStr`. Related edges: `NextHopAddress`'s `Display` renders a pair comma-joined (round-tripping with `FromStr`), and `BgpModelsError` gains a `NextHopParsingError` variant.
